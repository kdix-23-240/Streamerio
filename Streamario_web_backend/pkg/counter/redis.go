...
func (rc *redisCounter) keyCount(roomID, eventType string) string  { return fmt.Sprintf("room:%s:cnt:%s", roomID, eventType) }
func (rc *redisCounter) keyViewers(roomID string) string            { return fmt.Sprintf("room:%s:viewers", roomID) }
